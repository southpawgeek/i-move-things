...
function assertLevelDefinition(level: unknown): asserts level is JsonLevelDefinition {
...
function parseLevel(level: unknown): LevelDefinition {
  ...
  return {
    ...
    tiles: level.tiles.map((row) =>
      row.map((symbol) => TILE_SYMBOL_MAP[symbol as keyof typeof TILE_SYMBOL_MAP]),
    ),
  };
}
